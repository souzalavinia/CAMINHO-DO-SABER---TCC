...
<div class="main-content">
	...
	<div class="history-container">
		...
		<table class="history-table">
			...
			<tbody>
				<?php
				// Consulta ajustada: Usa INNER JOIN E FILTRA O CAMPO simulado = 'não'
				$sql = "
					SELECT
						p.id AS idProva,
						p.nome AS nomeProva,
					...
					INNER JOIN tb_prova p ON t.idProva = p.id
					WHERE 
						t.idUsuario = ? 
						AND p.simulado = 'não'  -- NOVO FILTRO: Apenas provas que NÃO são simulados
					GROUP BY p.id, p.nome, p.simulado
					ORDER BY p.nome ASC
				";
				
				$stmt_history = $conn->prepare($sql);
				
				// Se a preparação falhar, exibe o erro e encerra
				if ($stmt_history === false) {
				    echo "<tr><td colspan='2'>ERRO DE PREPARAÇÃO SQL: " . $conn->error . "</td></tr>";
                    $conn->close();
                    exit;
				}
				...
				// Se a execução falhar, exibe o erro e encerra
				if (!$stmt_history->execute()) {
				    echo "<tr><td colspan='2'>ERRO DE EXECUÇÃO SQL: " . $stmt_history->error . "</td></tr>";
                    $stmt_history->close();
                    $conn->close();
                    exit;
				}
				...
				if ($consulta && $consulta->num_rows > 0) {
					while ($dados = $consulta->fetch_assoc()) {
						...
						// Nesta lógica, como o SQL já filtrou p.simulado = 'não',
						// não precisamos mais do 'if ($dados['simulado'] === 'não')'.
						...
						// O botão é sempre exibido, pois a consulta só traz provas p.simulado = 'não'
						echo "		<a class='tentativas-chip' 
					...
					echo "<tr><td colspan='2'>Nenhuma prova *não-simulado* realizada ainda</td></tr>";
				}
